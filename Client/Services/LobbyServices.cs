...
public class LobbyServices
{
    ...
    private void RegisterHandlers()
    {
        ...
        _lobbyConnection.On<List<Game>>(nameof(ILobbyHubClient.GamesUpdated), games =>
        {
            ClientMemory.Games = games;
        });

        _lobbyConnection.On<Game, List<string>>(nameof(ILobbyHubClient.WaitingForPlayers), (game, playerNames) =>
        {
        ...
        _lobbyConnection.On<Game>(nameof(ILobbyHubClient.GameStarting), game =>
        {
            waitingAnim.Stop();
        ...
        _lobbyConnection.On<Game>(nameof(ILobbyHubClient.UpdateGameInProgressWhenPlayerQuits), game =>
        {
    ...
    public async Task CreateGameAsync()
    {
        ...
        ClientMemory.CurrentGame = await _lobbyConnection.InvokeAsync<Game>(nameof(ILobbyHubServer.CreateGame), command);
    ...
    public async Task DisplayAndJoinGameAsync()
    {
        ...
        if (int.TryParse(Console.ReadLine(), out int choice) && choice > 0 && choice <= ClientMemory.Games.Count)
        {
            ...
            ClientMemory.CurrentGame = await _lobbyConnection.InvokeAsync<Game?>(nameof(ILobbyHubServer.JoinGame), command);
    ...
    private async Task StartGameAsync()
    {
        ...
        while (ClientMemory.CurrentGame.Rounds.Count <= ClientMemory.CurrentGame.RoundsNumber)
        {
