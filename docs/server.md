...
`src/SeriousGame.Server` s'organise en couches Application / Infrastructure, au-dessus des modèles de domaine et DTOs partagés décrits dans [architecture.md](architecture.md).
...
- `Application/Services/` : `GameService`, `PlayerService` (logique métier, dépendent des interfaces de repository, pas directement de `AppMemory`), `GameFactory` (encapsule les valeurs par défaut de création d'un `Game` — pattern Factory), `LobbyFlowService` (l'**unique** implémentation du flux create/join/disconnect du lobby).
